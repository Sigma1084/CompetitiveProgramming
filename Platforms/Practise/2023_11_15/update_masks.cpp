...
private:
    ...
    void consume(int node) override {
        if (lazy[node]) {
            tree[node] = sizeOf(node) - tree[node];
        }
    }
};

string to_string(SegTree &sg, int n) {
    string res;
    for (int i = 0; i < n; ++i) {
        res.push_back(char(sg.query(i, i) + '0'));
    }
    return res;
}
...
int main() {
    ...
    // Initial
    assert(to_string(sg, n) == "00000");

    sg.update(1, 3, 1);
    assert(to_string(sg, n) == "01110");

    assert(sg.query(1, 2) == 2);

    sg.update(0, 4, 1);
    assert(to_string(sg, n) == "10001");

    assert(sg.query(3, 4) == 1);
