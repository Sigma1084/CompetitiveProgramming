...
using T = pair<int, int>;  // [minimum, frequency]
using U = int;
struct SegTree: LazySegTreeBase<T, U> {
    constexpr static T idElement = make_pair(1e9 + 7, 0);
    constexpr static U idUpdate = -1;  // Assignment
...
private:
    ...
    void consume(int node) override {
        tree[node] = make_pair(lazy[node], sizeOf(node));
    }
};
...
int main() {
    ...
    for (auto &[m, f]: a) cin >> m;
    st.assign(a);

    for (int t, l, r, i, v; nQ--; ) {
        cin >> t;
        if (t == 1) {
            cin >> i >> v;
            st.update(i, i, v);
