...
void solve() {
    ...
    mp.pop_back();  // Ignore the last element

    ll m;
    cin >> m;

    set<pair<int, int>> s;
    s.emplace(-2, -2);  // -1 interferes with 0
    s.emplace(n + 1, n + 1);  // n interferes with n - 1
    SegTree segTree(n);
    ll ans = 0;
    for (int i = 0; i < n; ++i) {
        for (auto idx: mp[i]) {

            // Find the first interval greater than idx
            auto it = s.upper_bound({idx, n});
            auto [rs, re] = *(it--);
            auto [ls, le] = *it;

            // 4 cases
            if (le + 1 != idx and idx + 1 != rs) {
                // Isolated
                s.insert({idx, idx});
            } else if (idx + 1 == rs) {
                // Merge with right
                s.erase({rs, re});
                s.insert({idx, re});
                segTree.update(rs, rs, 1);
            } else if (le + 1 == idx) {
                // Merge with right
                s.erase({ls, le});
                s.insert({ls, idx});
                segTree.update(idx, idx, 1);
            } else {
                // Merge both
                s.erase({ls, le});
                s.erase({rs, re});
                s.insert({ls, re});
                segTree.update(idx, rs, 1);
            }
        }

        ans += segTree.query(0, n - 1);
    }
