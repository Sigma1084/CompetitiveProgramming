...
void solve() {
    ...
    ll leftSum = a[left], rightSum = a[right];
    int count = 0;

    auto moveLeft = [&] () -> void { left++; leftSum += a[left]; };
    auto moveRight = [&] () -> void { right--; rightSum += a[right]; };
    auto updateCount = [&] () -> void { count = left+1 + n-right; };

    while (left < right) {
        if (leftSum == rightSum) updateCount(), moveLeft(), moveRight();
        else if (leftSum < rightSum) moveLeft();
        else moveRight();
    }
