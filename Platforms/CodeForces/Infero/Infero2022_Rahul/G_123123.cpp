...
void solve() {
    ...
    vvi prev(n, vi(3, -1));
    For(n) {
        if (i) for (int j=0; j<3; j++) prev[i][j] = prev[i-1][j];
        prev[i][s[i]-'1'] = i;
    }

    {
        vi l = vi(prev[n - 1]);
        if (l[0] == -1 or l[1] == -1 or l[2] == -1) { cout << 0; return; }
    }

    int ans = n;
    For(n) {
        vi cur = vi(prev[i]);
        if (cur[0] == - 1 or cur[1] == -1 or cur[2] == -1) continue;
        int newAns = max(i - cur[0], max(i- cur[1], i - cur[2]));
