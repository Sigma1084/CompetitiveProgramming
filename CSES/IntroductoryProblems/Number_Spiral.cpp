...
void solve() {
    ...
    // We can go to (pos, pos) and we know how to traverse from (pos, pos)

    // From (pos, pos), if pos is even, top is neg, else top is pos
    // From (pos, pos), is pos is even, left is pos, else left is neg
    
    int pos = max(r, c);
    ll ans = (ll)(pos-1)*(pos-1) + pos;
    if (r < pos) {
        if (pos%2  == 0) {
            ans -= (pos-r);
        } else {
