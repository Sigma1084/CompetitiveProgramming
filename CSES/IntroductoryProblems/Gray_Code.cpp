...
const int N = pow(2, 16) + 2;
vi ans(N, 0);

void update(int k) {
    int adder = pow(2, k-1);
    for (int i=adder, d=1; i<2*adder; i++, d++) {
        ans[i] = adder + ans[adder-d];
    }
}
...
void solve() {
    ...
    for (int i=0; i<pow(2, n); i++) {
        string binary;
        switch(n) {            
            case 1: binary = bitset<1>(ans[i]).to_string(); break;
            case 2: binary = bitset<2>(ans[i]).to_string(); break;
            case 3: binary = bitset<3>(ans[i]).to_string(); break;
            case 4: binary = bitset<4>(ans[i]).to_string(); break;
            case 5: binary = bitset<5>(ans[i]).to_string(); break;
            case 6: binary = bitset<6>(ans[i]).to_string(); break;
            case 7: binary = bitset<7>(ans[i]).to_string(); break;
            case 8: binary = bitset<8>(ans[i]).to_string(); break;
            case 9: binary = bitset<9>(ans[i]).to_string(); break;
            case 10: binary = bitset<10>(ans[i]).to_string(); break;
            case 11: binary = bitset<11>(ans[i]).to_string(); break;
            case 12: binary = bitset<12>(ans[i]).to_string(); break;
            case 13: binary = bitset<13>(ans[i]).to_string(); break;
            case 14: binary = bitset<14>(ans[i]).to_string(); break;
            case 15: binary = bitset<15>(ans[i]).to_string(); break;
            case 16: binary = bitset<16>(ans[i]).to_string(); break;
        }
        cout << binary << NL;
    }
}
