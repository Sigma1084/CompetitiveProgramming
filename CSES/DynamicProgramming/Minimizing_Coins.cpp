...
void solve() {
    int n, x; cin >> n >> x;
    vi a(n); cin >> a; sort(all(a));

    /**
     * @brief Explanation
     * 
     * Suppose dp[x] is the minimum number of coins required for money x <br>
     * Suppose dp[x] = INF if it can not be constructed <br>
     *
     * dp[x] = 1 + Minimum(dp[x-ci]) where dp[x-ci] != INF <br>
     * 
     * Required: dp[x]
     */
    ...
    vector<int> dp(x+1, INF); dp[0] = 0;  // 0 coins to construct 0
    for (int i=1; i<=x; i++) {
