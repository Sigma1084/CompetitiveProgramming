...
void solve() {
     ...
     * @brief Explaination
     * 
     * Suppose dp[i] is the number of ways of constructing i
     * 
     * If i <= 6, It is hard coded to be 2 ^ (i-1)
     * If i > 6, the possible ways are 
     * 1 + (i-1)
     * 2 + (i-2)
     * 3 + (i-3)
     * 4 + (i-4)
     * 5 + (i-5)
     * 6 + (i-6)
     * Therefore, dp[i] = dp[i-1] + dp[i-2] + ... dp[i-6]
     * 
     * Required Answer: dp[n]
     */

    vector<int> dp(max(7, n+1), 0); dp[1] = 1; 
    for (int i=2; i<=6; i++) dp[i] = dp[i-1] * 2;

    for (int i=7; i<=n; i++) for (int j=1; j<=6; j++) 
