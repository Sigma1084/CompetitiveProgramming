...
tempT istream &operator >> (istream &in,vector<T> &v) { 
...
void solve() {
    ...
    For(n) {
        int k, j; cin >> k >> j;
        if (k == j or e[k].size() == 2 or e[j].size() == 2) flag = true;
        e[k].pb(j);
        e[j].pb(k);
    }
    ...
    auto processLine = [&](auto && processLine, int u) {
        // Size of e[n] will be 1
        // u -> v (-> w)
        int v = e[u][0];
        e[u].clear();
        ...
        int w = e[v][0] == u ? e[v][1] : e[v][0];
        e[v].clear();
        e[v] = { w };

        processLine(processLine, v);
    };

    // When the same element does not Appear Twice
    For(n+1) {
        if (e[i].size() == 0) continue;
        if (e[i].size() == 1) { processLine(processLine, i); continue; }
