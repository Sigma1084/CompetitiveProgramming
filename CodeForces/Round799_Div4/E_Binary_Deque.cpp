...
void solve() {
    ...
    // auto f = [&] () -> void {};
    vi begin(1, -1); vi end(1, n);
    For(n) { if (a[i] == 1) begin.pb(i); }
    ForRev(n) { if (a[i] == 1) end.pb(i); }

    // begin[3] = 7 means u need to move to at least 7 to reach 3 ones
    // end[3] = 10 means u need to move to at least 10 to reach 3 ones

    // begin[0] = -1 u need not move at all
    // end[0] = n u need not move at all

    int ones = begin.size() - 1;
    if (ones < k) { cout << -1; return; }
    
    // In the end k is required
    // So we remove ones-k
    ...
    int ans = INT_MAX;
    for(int i=0; i <= k; i++) {
        ans = min(ans, begin[i]+1 + n-end[k-i]);
    }
