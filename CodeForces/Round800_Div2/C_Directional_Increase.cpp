...
void solve() {
    ...
    // Sum must be 0
    if (Sum(a) != 0) { cout << "No"; return; }
    while (a[n-1] == 0) n--;
    if (n == 0) { cout << "Yes"; return; }

    if (a[0] < 1) { cout << "No"; return; }

    // This means, last Element is non zero

    ll cur = -a[0] + 1;

    for (int i = 1; i < n - 1; i++) {
        // Decreasing is not an option
        ll req = a[i] + 1;
        if (cur >= req) { cout << "No"; return; } 
        // Cuz this is not the last non zero element

        // cur is less than req and hence, we can increase cur
        // we get one for free and the rest we subtract from the next number
        // a[i] will now be req after adding one 
        cur = -(req - cur + 1);
    }
