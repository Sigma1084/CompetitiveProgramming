...
void solve() {
    ...
    if (n%2 == 1) {
        ll ans = 0;
        for (int i=1; i<n; i+=2) {
            ans += max(0, 1 + max(a[i-1] - a[i], a[i+1] - a[i]));
        }
        cout << ans;
        return;
    }
    ...
    vi req(n);
    for (int i=1; i<n-1; i++) {
        req[i] = max(0, 1 + max(a[i-1] - a[i], a[i+1] - a[i]));
    }

    // Basically a few odds and a few evens
    vl oddSum; vl evenSum;
    oddSum.pb(0); evenSum.pb(0);
    for (int i=1; i<n-1; i+=2) {
        if (i == 1) oddSum.pb(req[i]);
        else oddSum.pb(oddSum[oddSum.size()-1] + req[i]);
    }
    for (int i=2; i<n-1; i+=2) {
        if (i == 2) evenSum.pb(req[i]);
        else evenSum.pb(evenSum[evenSum.size()-1] + req[i]);
    }

    ll ans = INF;
    int m = oddSum.size()-1;
    for (int i=0; i<=m; i++) {
        ans = min(ans, oddSum[i] + evenSum[m] - evenSum[i]);
    }
