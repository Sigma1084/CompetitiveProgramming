...
void solve() {
    ...
    if (k <= n) {
        ...
        ll s = cur;
        for (int i=k; i<n; i++) cur += a[i] - a[i-k]; s = max(s, cur);
        s += k * (k-1) / 2;
        cout << s;
    } else {
        auto tr = [](int n) -> ll { return (ll) n * (n+1) / 2; };
        ll ans = Sum(a);
        k--;
        if (n%2 == 1) {
            ans += 3 * tr(n/2);
            ...
            ll ans2 = 0; {
                ans2 += 3 * tr(n/2);
                k -= n;
                ans2 += (n+1) * min(k, n/2 - 1);
                k -= min(k, n/2 - 1);

                int full = k / (n-1);
                ans2 += (ll)full * 2 * tr(n-1);
                k %= (n-1);
                ans2 += (ll)2 * tr(k);
            }
            ans = max(ans1, ans2);
