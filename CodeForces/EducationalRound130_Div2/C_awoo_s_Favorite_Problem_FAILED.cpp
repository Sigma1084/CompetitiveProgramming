...
void solve() {
    ...
    // auto f = [&] () -> void {};
    int totalABs = 0;
    For(n) if (a[i] == 'b') totalABs++;
    int totalBBs = 0;
    For(n) if (b[i] == 'b') totalBBs++;
    if (totalABs != totalBBs) pN

    vi aAs; vi bAs;

    int temp = totalABs;
    For(n) { if (a[i] == 'a') aAs.pb(temp); if (a[i] == 'b') temp--; }
    temp = totalBBs;
    For(n) { if (b[i] == 'a') bAs.pb(temp); if (b[i] == 'b') temp--; }
    
    if (aAs.size() != bAs.size()) pN
    For(aAs.size()) { if (aAs[i] < bAs[i]) pN }

    vi aCs; vi bCs;
    temp = totalABs;
    For(n) { if (a[i] == 'c') aCs.pb(temp); if (a[i] == 'b') temp--; }
    temp = totalBBs;
    For(n) { if (b[i] == 'c') bCs.pb(temp); if (b[i] == 'b') temp--; }
    
    if (aCs.size() != bCs.size()) pN
    For(aCs.size()) { if (aCs[i] > bCs[i]) pN }
