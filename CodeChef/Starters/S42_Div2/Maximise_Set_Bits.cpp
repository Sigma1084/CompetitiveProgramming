...
void solve() {
    ...
    // auto f = [&] () -> void {};
    // Make n numbers using k

    ll ans = 0, p = 0;

    for (ll i = 1; true; i *= 2, p++) {
        if (i*n <= k) {
            ans += n;
            k -= i*n;
        } else {
    ...
    // numPos is the number of 1s contained in the last one 
    if (k) {
        ans -= p;
        ll newNum = pow(2, p) + (k-1);
        while (newNum) ans += newNum%2, newNum /= 2;
    }
