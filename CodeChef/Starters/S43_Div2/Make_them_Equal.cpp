...
void solve() {
    ...
    while (a > 0 or b > 0 or c > 0) {
        ...
        if (i+j+k == 3) break;  // No way to add
        if (i+j+k == 2) {}  // Add one to THAT number
        else if (i+j+k == 1) {
            // Only 1 number is 1, and we plan to add to that number
            // If some number is already 0, no point
            if (a == 0 and (b == 0 or c == 0)) break;
            if (b == 0 and c == 0) break;
            if (i) a++;
            if (j) b++;
            if (k) c++;
        }
        else break;  // No way to add
