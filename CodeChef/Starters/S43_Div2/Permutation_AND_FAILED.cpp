...
void solve() {
    int n; cin >> n;
    if (n == 1) { cout << 1 << NL << 1; return; }

    // auto f = [&] () -> void {};

    // We need either n = 2^k or n = 2^k - 2
    int big; bool is2 = false;
 
    if (isInt(log2(n))) {
        big = n-1;
        is2 = true;
    } else if (isInt(log2(n+2))) {
        big = n+1;
        is2 = false;
    } else {
        cout << -1; 
    ...
    vi a;
    for(int i=1; i<big; i++) a.pb(i ^ big);
    if (is2) a.pb(big+1), a.pb(big);
