...
void solve() {
    ...
    // auto f = [&] () -> void {};
    // For any even number it can be done

    // Handle power of 2 separately
    vi f(n);
    int end = n;
    int req;

    auto handlePow2 = [&]() {
        for (int i=0; i<end-2; i++) f[i] = end-2-i;
        f[end-2] = end; f[end-1] = end-1;
    };
    ...
    auto doF = [&]() -> int {
        ...
        int retInt = req - end - 1;
        for (int i = retInt+1; i <= end; i++) {
            f[i-1] = req - i;
        }
        return retInt;
    };
    ...
    do {
        req = pow(2, ceil(log2(end))) - 1;
        end = doF();
    } while(end > 0);

    For(n) cout << i+1 << ' '; cout << NL;
    Trav(f) cout << i << ' ';
}
