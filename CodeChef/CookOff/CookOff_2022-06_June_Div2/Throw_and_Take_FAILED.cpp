...
void solve() {
    ...
    vi a; {
        ...
        For(n) if (c[i] % 2 == 1) a.pb(v[i]);
    }

    if (a.empty()) { cout << 0; return; }
    if (a.size() == 1) { cout << a[0]; return; }

    // At least 2 elements
    n = a.size();
    a.pb(MOD);  // a[n] is very large

    vl dp(n+2);
    dp[n] = dp[n+1] = 0;
    dp[n-1] = a[n-1];

    vi maxInd(n); maxInd[n-1] = n-1;
    vi sMaxInd(n); sMaxInd[n-1] = n;

    for (int i=n-2; i>=0; i--) {
        if (a[i] >= a[maxInd[i+1]]) {
            maxInd[i] = i;
            sMaxInd[i] = maxInd[i+1];
        } else {
            maxInd[i] = maxInd[i+1];
            sMaxInd[i] = sMaxInd[i+1];
        }
    }

    for (int i=n-2; i>=0; i--) {
        dp[i] = max(dp[i+1], a[maxInd[i]] - a[sMaxInd[i]] + dp[sMaxInd[i] + 1]);
    }
