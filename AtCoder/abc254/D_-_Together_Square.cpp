...
const int N = 450;  // 450 * 450 = 202500
vi primes;

void seive() {
    vb isPrime(N+1, true); isPrime[0] = isPrime[1] = false;
    for(int i=2; i*i <= N; i++) {
        if (!isPrime[i]) continue;
        for(int j=i*i; j <= N; j+=i) isPrime[j] = false; 
    }
    For(N) if(isPrime[i]) primes.pb(i);
}
...
void solve() {
    ...
    ll ans = 0;
    for(int i=1; i <= n; i++) {
        // To calculate all the j's that satisfy the condition
        int k = i;
        for(auto p: primes) {
            if (p*p > k) break;
            while (k % (p*p) == 0) k /= p*p;
        }
        // we need at least leastPrime
        for(int j=1; j*j*k <= n; j++, ans++);
    }
